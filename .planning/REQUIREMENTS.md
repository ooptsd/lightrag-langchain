...
- [x] **LLM-04**: LLM 关键词提取 — 从用户查询中提取 high-level keywords（宏观主题）和 low-level keywords（具体实体）
- [x] **LLM-05**: Token 预算控制 — 动态分配 entity/relation/chunk 的 token 配额，max_entity_tokens + max_relation_tokens < max_total_tokens，剩余 token 分配给 chunks
